...
import { Plus, Trash2, Scroll, Shield, User, Star, LayoutGrid } from "lucide-react";
...
const ScionInput = ({ label, className, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label?: string }) => (
        ...
        <div className="flex justify-between items-end mb-8 border-b border-primary/20 pb-4">
            <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2">
                    <span className="font-tech text-xs text-muted-foreground tracking-[0.5em] uppercase">Admin Console v1.0</span>
                </div>
            </div>
            <Link href="/">
              <button className="flex items-center gap-2 text-[10px] font-mythic uppercase tracking-widest text-primary border border-primary/30 px-3 py-1 hover:bg-primary/10 transition-colors rounded-sm">
                <LayoutGrid className="w-3 h-3" /> View Character Sheet
              </button>
            </Link>
        </div>
